...
import com.example.compose.jetchat.data.CitiesLocalDataSource
...
fun launchComposeMapActivity(context: Context) {
    context.startActivity(createMapActivityIntent(context))
}

@VisibleForTesting
fun createMapActivityIntent(context: Context): Intent {
    return Intent(context, ComposeMapActivity::class.java)
}
...
private data class MapScreenUiState(
    val city: City? = null,
...
fun MapScreen(
    ...
    viewModel: MapViewModel = MapViewModel(DestinationsRepository(CitiesLocalDataSource()))
) {
    ...
    val uiState by produceState(
        key1 = viewModel,
        initialValue = MapScreenUiState(isLoading = true)
    ) {
        val cityMapResult = viewModel.cityDetails
        value = if (cityMapResult is Result.Success<City>) {
            MapScreenUiState(cityMapResult.data)
        } else {
            MapScreenUiState(throwError = true)
        }
    }
    ...
    Crossfade(targetState = uiState, modifier) { currentUiState ->
        when {
            currentUiState.city != null -> {
                MapContent(currentUiState.city, Modifier.fillMaxSize())
            }
